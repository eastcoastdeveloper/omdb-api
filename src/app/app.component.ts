...
export class AppComponent implements OnInit, OnDestroy {
  ...
  fetchMultipleFlicks(movie: string) {
    this._http.get<Movie>(`${this.baseUrl} ${movie} ${environment.key}`).pipe(takeUntil(this.unsubscribe$))
      .subscribe((data: any) => {
  ...
  fetchResults() {
    ...
    this._http
      .get<Movie>(`${this.baseUrl} ${this.titleQuery} &y= ${this.yearQuery} ${environment.key}`)
